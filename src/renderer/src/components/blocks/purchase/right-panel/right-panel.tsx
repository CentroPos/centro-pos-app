...
const PrintsTabContent: React.FC = () => {
  ...
  const { getCurrentTab } = usePurchaseTabStore()
...
const RightPanel: React.FC<RightPanelProps> = ({
  ...
  const [productSubTab, setProductSubTab] = useState<'purchase-history' | 'supplier-history'>(
    'purchase-history'
  )
  ...
  const productTabs = [
    { id: 'purchase-history', label: 'Purchase History', color: 'blue' },
    { id: 'supplier-history', label: 'Supplier History', color: 'emerald' }
  ]
  ...
  useEffect(() => {
    ...
    console.log('🔄 Selected Supplier:', selectedCustomer)
    console.log('🔄 Supplier ID:', selectedCustomer?.supplier_id || selectedCustomer?.name)
              ...
              <div className="bg-white/90 mt-2">
                <div className={`flex border-b border-gray-200/60 ${shouldScrollTabs ? 'overflow-x-auto scrollbar-hide scroll-smooth' : ''}`}>
                  <div className={`flex ${shouldScrollTabs ? 'min-w-max' : 'w-full'}`}>
                    ...
                    <button
                      className={`px-4 py-3 font-bold text-sm border-b-2 ${shouldScrollTabs ? 'whitespace-nowrap' : 'flex-1'} ${productSubTab === 'supplier-history'
                        ? 'border-emerald-500 bg-emerald-50 text-emerald-700'
                        : 'text-gray-500 hover:text-black hover:bg-white/40'
                        }`}
                      onClick={() => {
                        console.log('🔄 Switching to Supplier History tab')
                ...
                {productSubTab === 'purchase-history' && (
                  <div className="p-4">
                    <div className="text-xs text-gray-500 mb-2">
                      Purchase history for {selectedItem?.item_name || selectedItemId || 'selected product'}
                    </div>
                        ...
                        <div className="space-y-2">
                          {purchaseHistory
                            .filter((item: any) => {
                              ...
                              const searchLower = purchaseHistorySearch.toLowerCase()
                              return (
