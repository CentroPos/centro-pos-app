...
const ItemsTable: React.FC<Props> = ({ selectedItemId, onRemoveItem, selectItem, shouldStartEditing = false, onEditingStarted, onAddItemClick, onSaveCompleted, isProductModalOpen = false, isCustomerModalOpen = false, isErrorBoxFocused = false, onEditingStateChange, errorItems = [] }) => {
  const { getCurrentTabItems, activeTabId, updateItemInTab, updateItemInTabByIndex, getCurrentTab, setTabEdited, removeItemFromTabByIndex, updateTabOtherDetails, updateTabPostingDate, updateTabOrderData, updateTabCustomer, updateTabDiscountMode } = usePurchaseTabStore();
  ...
  useEffect(() => {
    ...
    const fetchMissingRates = async () => {
      ...
      for (const { item, index: _index } of itemsToFetch) {
  ...
  */
  ...
  const hidePurchaseItemDiscounts = Boolean(profile?.custom_hide_purchase_item_discounts === 1);
        ...
        <div className="relative flex-1 min-h-0">
          <TabsContent value="items" className="absolute inset-0 flex flex-col min-h-0 data-[state=inactive]:hidden">
            <div className="border rounded-lg flex flex-col min-h-0">
              ...
              >
                <table className="w-full min-w-[1150px] text-sm table-fixed border-collapse">
                        ...
                        >
                          ...
                          {!hidePurchaseItemDiscounts && (
                            ...
                            <select
                              ...
                              className="w-full text-[11px] py-0.5 px-0 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white disabled:bg-gray-100 disabled:text-gray-500 cursor-pointer text-center"
                              onClick={(e) => e.stopPropagation()}
                              onMouseDown={(e) => e.stopPropagation()}
                              onChange={(e) => {
                            ...
                            >
                              ...
                              <option value="Amount">{profile?.custom_currency_symbol || '$'}</option>
                            </select>
                          </TableCell>
                            ...
                            {(() => {
                              ...
                              if (item.discount_type === 'Amount') {
                                ...
                                if (effectiveDiscountMode !== 'Row Total') {
                                  discountAmt = flt(discountAmt * rawQty)
                                }
                              } else {
