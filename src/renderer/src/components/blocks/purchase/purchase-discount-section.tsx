...
const PurchaseDiscountSection: React.FC<Props> = ({
  ...
  const hidePurchaseAdditionalDiscount = Boolean(profile?.custom_hide_purchase_additional_discount === 1);
  ...
  useHotkeys(
    'ctrl+d',
    () => {
      if (isReadOnly || hidePurchaseAdditionalDiscount) return
      if (currentTab) {
  ...
  return (
    <div className="p-2 pb-6 relative">
      <div className="flex gap-3 mb-2">
        {!hidePurchaseAdditionalDiscount && (
          <Button
            variant="outline"
